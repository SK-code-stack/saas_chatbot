    ...
    <div className="min-h-screen bg-[#0b1326] text-[#dae2fd] flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 p-4 md:p-8 space-y-8 overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#2d3449] pb-6">
            <div>
              <h1 className="text-xl md:text-2xl font-bold text-white tracking-tight">Billing & Stripe Subscription</h1>
              <p className="text-xs md:text-sm text-[#908fa0]">Manage subscription tiers, payment methods, and download invoice receipts.</p>
            </div>
            <button
          ...
          <div className="bg-[#171f33] border border-[#2d3449] rounded-2xl p-6 space-y-6 relative overflow-hidden">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#2d3449] pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#908fa0]">$99.00 / month billed via Stripe</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[#908fa0]">Monthly Message Quota</span>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-[#908fa0]">Vector Storage Space</span>
          ...
          <div className="bg-[#171f33] border border-[#2d3449] rounded-2xl p-6 space-y-6">
            <div className="flex items-center justify-between border-b border-[#2d3449] pb-4">
              <div>
                ...
                <p className="text-xs text-[#908fa0]">Manage credit cards used for auto-billing</p>
              </div>
              <button
                onClick={() => setShowAddCardModal(true)}
                className="px-4 py-2 rounded-xl bg-[#222a3d] border border-[#31394d] hover:bg-[#31394d] text-white text-xs font-semibold flex items-center gap-1.5 transition-colors"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {paymentMethods.map((pm) => (
                <div key={pm.id} className="bg-[#131b2e] border border-[#2d3449] rounded-xl p-4 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-[#222a3d] border border-[#31394d] flex items-center justify-center text-xs font-bold text-white">
                      {pm.brand}
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                      ...
                      <p className="text-[10px] text-[#908fa0]">Expires {pm.exp}</p>
                    </div>
                  </div>
          ...
          <div className="bg-[#171f33] border border-[#2d3449] rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between border-b border-[#2d3449] pb-4">
              ...
              <span className="text-xs text-[#908fa0] font-mono">3 Invoices Paid</span>
            </div>
                ...
                <tbody className="divide-y divide-[#2d3449]/30 text-[#dae2fd]">
                  {invoices.map((inv, idx) => (
                    <tr key={idx} className="hover:bg-[#222a3d]/40 transition-colors">
                      ...
                      <td className="py-3.5 text-[#908fa0]">{inv.date}</td>
                      <td className="py-3.5 font-bold text-white">{inv.amount}</td>
                      <td className="py-3.5 text-[#c7c4d7]">{inv.card}</td>
                      <td className="py-3.5">
      ...
      {showAddCardModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm">
          <div className="bg-[#171f33] border border-[#2d3449] rounded-2xl p-6 w-full max-w-md space-y-4 shadow-2xl relative">
            <div className="flex items-center justify-between border-b border-[#2d3449] pb-3">
              ...
              <button onClick={() => setShowAddCardModal(false)} className="text-[#908fa0] hover:text-white">
            ...
            <form onSubmit={handleAddCardSubmit} className="space-y-4">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-[#c7c4d7]">Cardholder Name</label>
                <input
                  type="text"
                  required
                  value={cardForm.name}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-[#c7c4d7]">Card Number</label>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-[#131b2e] text-white text-xs sm:text-sm font-mono pl-3.5 pr-10 py-2.5 rounded-xl border border-[#2d3449] outline-none"
                  />
                  <span className="material-symbols-outlined absolute right-3 top-2.5 text-[#908fa0]">credit_card</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-[#c7c4d7]">Expiry Date</label>
                  <input
                    type="text"
                    required
                    placeholder="MM / YY"
                    ...
                    className="w-full bg-[#131b2e] text-white text-xs font-mono px-3.5 py-2.5 rounded-xl border border-[#2d3449] outline-none"
                  />
                </div>
                <div className="space-y-1.5">
                  <label className="text-xs font-medium text-[#c7c4d7]">CVC / CVV</label>
                  <input
                    type="text"
                    required
                    maxLength="4"
